...
const electronBinaryPath = (() => {
  ...
  return path.join(
    sourceRootDir,
    "node_modules/electron/dist/electron" + exeExtension
  );
})();
...
const mattermostURL =
  process.env.MM_TEST_SERVER_URL || "http://localhost:8065/";
...
module.exports = {
      ...
      ps.lookup(
        ...
        (err, resultList) => {
          ...
          resultList.forEach((process) => {
            if (
              process &&
              process.command === electronBinaryPath &&
              !process.arguments.some((arg) => arg.includes("electron-mocha"))
            ) {
              ps.kill(process.pid);
            }
          });
  ...
  async openDownloadsDropdown(app) {
    ...
    const dlButtonLocator = await mainWindow.waitForSelector(
      ".DownloadsDropdownButton"
    );
    await dlButtonLocator.click();
    await asyncSleep(500);

    const downloadsWindow = app
      .windows()
      .find((window) => window.url().includes("downloadsDropdown.html"));
